...
#define quick_fn_register(n, f)                                                                                        \
    lua_pushstring(L, n), lua_pushlightuserdata(L, (void *)pFiles), lua_pushcclosure(L, f, 1),                         \
...
int CLuaFile2::_luaInherit(lua_State *L) { return _getOwner(L)->_luaParent("Inherit", "GameData"); }

int CLuaFile2::_luaInheritMeta(lua_State *L) { return _getOwner(L)->_luaParent("InheritMeta", "MetaData"); }

int CLuaFile2::_luaReference(lua_State *L) { return _getOwner(L)->_luaParent("Reference", "GameData"); }
    ...
    try
    {
        ...
        if ((oParentFile.L = m_pCache->Fetch(sFileName)) == nullptr)
        {
            ...
            else
            {
                ...
                try
                {
                    m_pCache->AddToCache(sFileName, oParentFile.L);
                }
                IGNORE_EXCEPTIONS
            }
        }
...
unsigned long CLuaStateNode::VGetValueInteger() { QUICK_THROW("Wrong type") }
...
const wchar_t *CLuaStateNode::VGetValueWString(){QUICK_THROW("Wrong type")}
...
IMetaNode::eDataTypes CLuaStateTable::VGetReferenceType() { return IMetaNode::DT_NoData; }
...
void CLuaStateStackNode::SGetNodeFromRainmanRgd(IFileStore::IStream *pInput, bool bSetName)
{
    QUICK_THROW("Unsupported");
}

CLuaFile2::CNode::CNode(lua_State *Lua, int iKey, int iTable) : m_oTablePtr(Lua, iTable), m_oKeyPtr(Lua, iKey) {}
CLuaFile2::CNode::CNode(lua_State *Lua, CLuaFile2::_LuaLocator *pTable, CLuaFile2::_LuaLocator *pKey)
    : m_oTablePtr(pTable), m_oKeyPtr(pKey)
{
}
...
unsigned long CLuaFile2::CNode::VGetValueInteger() { QUICK_THROW("Wrong type") }
...
const wchar_t *CLuaFile2::CNode::VGetValueWString(){QUICK_THROW("Wrong type")}
...
void CLuaFile2::CNode::_pushTable() { m_oTablePtr.push(L); }
...
void CLuaFile2::CNode::VSetType(eDataTypes eType)
{
    ...
    case IMetaNode::DT_Bool:
        iTargetLuaType = LUA_TBOOLEAN;
        break;
    case IMetaNode::DT_Float:
        iTargetLuaType = LUA_TNUMBER;
        break;
    case IMetaNode::DT_String:
        iTargetLuaType = LUA_TSTRING;
        break;
    case IMetaNode::DT_Table:
        iTargetLuaType = LUA_TTABLE;
        break;
    default:
...
CMemoryStore::COutStream *CLuaFile2::CNode::VGetNodeAsRainmanRgd()
{
    QUICK_THROW("TODO")
}
...
void CLuaFile2::_saveTable(const char *sPrefix, IFileStore::IOutputStream *pStream, int iMode)
{
    ...
    */
    ...
    for (auto itr = vKeys.begin(); itr != vKeys.end(); ++itr)
    {
        ...
        {
        case 1:
            lua_pushnumber(L, (**itr).f);
            break;
        case 2:
            lua_pushstring(L, (**itr).s);
            break;
        case 3:
