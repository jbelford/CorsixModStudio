...
class RAINMAN_API CSgaFile : public IFileStore, public IDirectoryTraverser
{
  public:
    ...
    /*!
        ...
        \param[in] oWriteTime The last modification date of the archive file. Older (DoW) SGA archives do not store the
       last modification date of each file in the archive, and will use this value for each file.
    ...
    */
    ...
    /*!
        Gets the input stream passed to VInit(). However, if VInit() threw an exception then the value is not
       remembered.
    ...
    */
    ...
    /*!
        \param[in] pInputStream Must be a IFileStore::IStream* . This pointer is remembered, and then used whenever a
       file inside the SGA archive is opened - as such do not delete the pointer until you are finished using the SGA
       archive. You can use GetInputStream() to get this pointer and delete it right before you delete this class.
...
#pragma pack(pop, sga_formats)
    ...
    /*!
        ...
        The CRC is case insensitive and thus uses a slightly modified version of the CRC algorithm (which is just as
       fast as a normal CRC).
    */
    struct _SgaDirHash
    {
        uint32_t iCRC;
