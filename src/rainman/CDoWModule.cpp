...
const char *CDoWModule::GetLocale() const { return m_sLocale; }
...
const char *CDoWModule::GetFilesModName() const { return m_sModFileName; }
...
void CDoWModule::Load(const char *sFile, CALLBACK_ARG)
{
	...
	if (sModuleFilename = strstr(m_sModFileName, ".module"))
		*sModuleFilename = 0;
	try
	{
	...
	if (!m_bLoadFSToCustom)
	{
		...
		if (!m_bNoEngine)
		{
			...
			if (pEngineDirItr)
				m_pFiles->AddFileSource(m_pFSO, pEngineDirItr, m_pFSO, GetEngineModName(), "Data", true);
			...
			if (m_bIsCohMod)
			{
				...
				try
				{
					...
					pDirItr = pEngineSga->VIterate(pEngineSga->VGetEntryPoint(0));
					if (pDirItr)
						m_pFiles->AddFileSource(pEngineSga, pDirItr, pEngineSga, GetEngineModName(), "RelicOnline.sga",
						                        true);
					delete pDirItr;
				}
				catch (CRainmanException *pE)
...
const char *CDoWModule::GetUIName() const { return m_sUIName; }

const char *CDoWModule::GetDescription() const { return m_sDescription; }

const char *CDoWModule::GetDllName() const { return m_sDllName; }

const char *CDoWModule::GetModFolder() const { return m_sModFolder; }

const char *CDoWModule::GetTextureFE() const { return m_sTextureFE; }

const char *CDoWModule::GetTextureIcon() const { return m_sTextureIcon; }
...
#define AUTO_SET(valname)                                                                                              \
	char *sTmp = CHECK_MEM(mystrdup(sValue ? sValue : ""));                                                            \
	if (valname)                                                                                                       \
		delete[] valname;                                                                                              \
	valname = sTmp;
...
unsigned long CDoWModule::_MakeFileSourcesHash(unsigned long iBase)
{
	...
	if (m_sModFolder)
		iBase = crc32(iBase, (const Bytef *)m_sModFolder, strlen(m_sModFolder));
	...
	for (std::map<long, char *>::iterator itr = m_mapArchiveFiles.begin(); itr != m_mapArchiveFiles.end();
	     ++itr, ++itrsga)
	{
		lval = 'GSGA';
		iBase = crc32(iBase, (const Bytef *)&lval, 4);
	...
	for (std::map<long, char *>::iterator itr = m_mapRequiredMods.begin(); itr != m_mapRequiredMods.end();
	     ++itr, ++itrmod)
	{
		lval = 'RQUM';
		iBase = crc32(iBase, (const Bytef *)&lval, 4);
