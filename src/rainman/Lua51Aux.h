...
*/
void  lua51L_openlib (lua_State *L, const char *libname,
                      const lua51L_Reg *l, int nup);
void  lua51L_register (lua_State *L, const char *libname,
                       const lua51L_Reg *l);
...
const char *lua51L_optlstring (lua_State *L, int numArg,
...
int   lua51L_checkoption (lua_State *L, int narg, const char *def,
...
int   lua51L_loadbuffer (lua_State *L, const char *buff, size_t sz,
                         const char *name);
int   lua51L_loadstring (lua_State *L, const char *s);
lua_State *lua51L_newstate (void);
const char *lua51L_gsub (lua_State *L, const char *s, const char *p,
                         const char *r);
const char *lua51L_findtable (lua_State *L, int idx,
...
#define lua51L_addchar(B,c) \
    ((void)((B)->p < ((B)->buffer+LUA51L_BUFFERSIZE) || lua51L_prepbuffer(B)), \
     (*(B)->p++ = (char)(c)))
#define lua51L_putchar(B,c)         lua51L_addchar(B,c)
#define lua51L_addsize(B,n)         ((B)->p += (n))

#define lua51_ref(L,lock) ((lock) ? lua51L_ref(L, LUA51_REGISTRYINDEX) : \
