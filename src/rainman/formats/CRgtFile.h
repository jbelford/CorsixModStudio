...
class RAINMAN_API CRgtFile
{
  ...
  protected:
	...
	static void _Save_Tga_Header(IFileStore::IOutputStream *pFile, unsigned short iWidth, unsigned short iHeight,
	                             bool bAlpha = true);

	void _Load_Dxtc();
	void _Save_Dxtc(IFileStore::IOutputStream *pFile);
	static void _Save_Dxtc_Header(IFileStore::IOutputStream *pFile, unsigned short iWidth, unsigned short iHeight,
	                              int iCompression, unsigned long iPrimaryLen, unsigned long iMipCount);

	static unsigned char *TranscodeData(unsigned char *pIn, unsigned long iInByteLen, unsigned char iBppIn,
	...
	static void _MakeDxtcDataTexFormat(CChunkyFile::CChunk *pParentChunk, unsigned long iWidth, unsigned long iHeight,
	                                   int iDxtN);
	static void _MakeDxtcData(CMemoryStore::COutStream *pTMAN, CMemoryStore::COutStream *pTDAT,
	                          IFileStore::IStream *pDDS, unsigned long iWidth, unsigned long iHeight, int iDxtN,
	                          bool bMipMaps, int iThisMipLevel = 0);
	static void _MakeTgaImagMips(CChunkyFile::CChunk *pTXTR, unsigned long iWidth, unsigned long iHeight,
