...
{
    m_pL = nullptr;
    m_pHashTableToUse = nullptr;
    m_pCallbackTag = nullptr;
    m_fpPrintCallback = nullptr;
    m_fpOnSaveCallback = nullptr;
    m_pUcsResolverModule = nullptr;
    m_bIsDowMod = true;
}

CRgdFileMacro::~CRgdFileMacro() { _cleanLua(); }
...
int CRgdFileMacro::_tCRgdFile::luaf_GET(lua_State *L)
{
    ...
    if ((lua51_tostring(L, 2) == nullptr) || stricmp(lua51_tostring(L, 2), "GameData") != 0)
        return 0;
    ...
    for (int n = 3; n <= iTop; ++n)
    {
        if ((pCurrentEntry->Type != IMetaNode::DT_Table) && (pCurrentEntry->Type != CRgdFile::sk_TableInt))
            return 0;
        if (pCurrentEntry->Data.t == nullptr)
            return 0;
...
int CRgdFileMacro::_tCRgdFile::luae_delayload(lua_State *L)
{
    ...
    lua51_newtable(L);
    lua51_settable(L, -3);
    lua51_pushstring(L, "__call");
    lua51_pushcfunction(L, luae_call);
    lua51_settable(L, -3);
                                                                ...
                                                                const char *_sFile, IFileStore *_pStore)
{
    ...
    lua51_pushstring(L, "GET");
    lua51_pushcfunction(L, luaf_GET);
    lua51_settable(L, -3);
    lua51_pushstring(L, "SET");
    lua51_pushcfunction(L, luaf_SET);
    lua51_settable(L, -3);
    lua51_pushstring(L, "save");
    lua51_pushcfunction(L, luaf_save);
    lua51_settable(L, -3);
    lua51_pushstring(L, "saveAs");
    lua51_pushcfunction(L, luaf_saveAs);
    lua51_settable(L, -3);
...
int CRgdFileMacro::luaf_pcall(lua_State *L)
{
    ...
    if (status != 0 && lua51_type(L, 2) == LUA_TLIGHTUSERDATA)
    {
        ...
        for (const CRainmanException *e = pE; e; e = e->getPrecursor())
        {
            ...
            lua51_pushnumber(L, (lua_Number)e->getLine());
            lua51_tostring(L, -1);
            lua51_pushstring(L, ": ");
...
CRgdFile::_RgdEntry *CRgdFileMacro::_getRootEntry(CRgdFile *pFile) { return &(pFile->m_pDataChunk->RootEntry); }
...
int CRgdFileMacro::_tCRgdTable::luae_newindex(lua_State *L)
{
    ...
    pNewChild->sName = pUs->pParentFile->GetHashTable()->HashToValue(
        pNewChild->iHash = pUs->pParentFile->GetHashTable()->ValueToHash(lua51_tostring(L, 2)));
    pNewChild->Type = CRgdFile::DT_Bool;
    pNewChild->Data.b = false;
    pNewChild->pExt = nullptr;

    return luae_setindex(L);
}

void CRgdFileMacro::setUcsResolver(CModuleFile *pModule) { m_pUcsResolverModule = pModule; }

void CRgdFileMacro::setIsDowMod(bool bIsDowMod) { m_bIsDowMod = bIsDowMod; }
        ...
        if ((**itr).iHash == iChildHash)
        {
            ...
            if (lua51_type(L, 3) == LUA_TNIL)
            {
                ...
                case CRgdFile::DT_String:
                    if (pChild->Data.s)
                        delete[] pChild->Data.s;
                    break;
                case CRgdFile::DT_WString:
                    if (pChild->Data.ws)
                        delete[] pChild->Data.ws;
                    break;
                };
                delete pChild;
...
int CRgdFileMacro::_tCRgdTable::luae_delayload(lua_State *L)
{
    ...
    for (auto itr = pThis->pRgdData->Data.t->begin(); itr != pThis->pRgdData->Data.t->end(); ++itr)
    {
        ...
        else
        {
            ...
            for (int n = 0; n < 8; ++n)
                sHex[9 - n] = "0123456789ABCDEF"[((**itr).iHash >> (n * 4)) & 0xF];
        ...
        {
            construct(L, nullptr, *itr); // T K V Tmeta ChildrenT RgdFileT key val
        }
    ...
    lua51_pushstring(L, "__newindex");
    lua51_pushcfunction(L, luae_newindex);
    lua51_settable(L, -3);
    lua51_pushstring(L, "__updateindex");
    lua51_pushcfunction(L, luae_setindex);
    lua51_settable(L, -3);
    lua51_pushstring(L, "__metatable");
    lua51_newtable(L);
    lua51_settable(L, -3);
...
CRgdFileMacro::_tCRgdTable *CRgdFileMacro::_tCRgdTable::construct(lua_State *L, _tCRgdFile *pRgdFile,
                                                                  CRgdFile::_RgdEntry *pRgdData)
{
    ...
    else
    {
        ...
        case IMetaNode::DT_Float:
            lua51_pushnumber(L, (lua_Number)pRgdData->Data.f);
            break;
        ...
        case IMetaNode::DT_Bool:
            lua51_pushboolean(L, pRgdData->Data.b ? 1 : 0);
            break;
        case IMetaNode::DT_String:
            lua51_pushstring(L, pRgdData->Data.s);
            break;
...
int CRgdFileMacro::luaf_print(lua_State *L)
{
    ...
    for (i = 1; i <= n; i++)
    {
        ...
        if (s == nullptr)
            return lua51L_error(L, "`tostring' must return a string to `print'");
        // pThis->_onPrint(s);
...
int CRgdFileMacro::luaf_next(lua_State *L)
{
    ...
    if (lua51_gettop(L) < 2)
        lua51_settop(L, 2); /* create a 2nd argument if there isn't one */
    if (lua51_next(L, 1))
        return 2;
    else
    {
...
{
    if (m_fpPrintCallback)
        m_fpPrintCallback(m_pCallbackTag, sMsg);
}

bool CRgdFileMacro::_onSave(const char *sFile)
{
    if (m_fpOnSaveCallback)
        return m_fpOnSaveCallback(m_pCallbackTag, sFile);
    return true;
}

bool CRgdFileMacro::_onLoad(const char *sFile)
{
    if (m_fpOnLoadCallback)
        return m_fpOnLoadCallback(m_pCallbackTag, sFile);
    return true;
}
...
void CRgdFileMacro::loadMacro(const char *sCode)
{
    ...
    lua51_call(m_pL, 0, 0);
    lua51_pushcfunction(m_pL, lua51open_math);
    lua51_call(m_pL, 0, 0);
    lua51_pushcfunction(m_pL, lua51open_io);
    lua51_call(m_pL, 0, 0);
    lua51_pushcfunction(m_pL, lua51open_os);
    lua51_call(m_pL, 0, 0);
    lua51_pushcfunction(m_pL, lua51open_string);
    lua51_call(m_pL, 0, 0);
    lua51_pushcfunction(m_pL, lua51open_debug);
    lua51_call(m_pL, 0, 0);
    ...
    lua51_pushstring(m_pL, "split");
    lua51_pushcclosure(m_pL, luax51_string_split, 0);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "after");
    lua51_pushcclosure(m_pL, luax51_string_after, 0);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "afterlast");
    lua51_pushcclosure(m_pL, luax51_string_afterlast, 0);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "before");
    lua51_pushcclosure(m_pL, luax51_string_before, 0);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "beforelast");
    lua51_pushcclosure(m_pL, luax51_string_beforelast, 0);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "formatself");
    lua51_pushcclosure(m_pL, luaf_formatme, 0);
    lua51_settable(m_pL, -3);
    lua51_pop(m_pL, 1);

    lua51_getglobal(m_pL, "debug");
    lua51_pushstring(m_pL, "debug");
    lua51_pushnil(m_pL);
    lua51_settable(m_pL, -3);
    lua51_pushnil(m_pL);
    ...
    lua51_pushstring(m_pL, "stdin");
    lua51_pushnil(m_pL);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "stdout");
    lua51_pushnil(m_pL);
    lua51_settable(m_pL, -3);
    lua51_pushstring(m_pL, "stderr");
    lua51_pushnil(m_pL);
    lua51_settable(m_pL, -3);
    lua51_pushnil(m_pL);
    while (lua51_next(m_pL, -2) != 0)
    ...
    if (iLoadErr)
    {
        ...
        else
        {
            ...
            {
            case LUA_ERRRUN:
                sErrM = "Runtime error loading macro : %s";
                break;
            case LUA_ERRMEM:
                sErrM = "Memory error loading macro : %s";
                break;
            case LUA_ERRERR:
                sErrM = "Error error loading macro : %s";
                break;
            default:
                sErrM = "Unknown error loading macro : %s";
                break;
            }
            PAUSE_THROW(nullptr, __FILE__, __LINE__, sErrM, lua51_tostring(m_pL, -1));
            lua51_pop(m_pL, 1);
            UNPAUSE_THROW;
        }
    }
...
void CRgdFileMacro::runAtEnd()
{
    ...
    if (iErr = lua51_pcall(m_pL, 0, 0, 0)) // NOLINT(bugprone-assignment-in-if-condition)
    {
        ...
        else
        {
            ...
            {
            case LUA_ERRRUN:
                sErrM = "Runtime error running end event : %s";
                break;
            case LUA_ERRMEM:
                sErrM = "Memory error running end event : %s";
                break;
            case LUA_ERRERR:
                sErrM = "Error error running end event : %s";
                break;
            default:
                sErrM = "Unknown error running end event : %s";
                break;
            }
            PAUSE_THROW(nullptr, __FILE__, __LINE__, sErrM, lua51_tostring(m_pL, -1));
            lua51_pop(m_pL, 1);
...
void CRgdFileMacro::runMacro(const char *sFile, IFileStore *pStore)
{
    ...
    if (iErr = lua51_pcall(m_pL, 1, 0, 0)) // NOLINT(bugprone-assignment-in-if-condition)
    {
        ...
        else
        {
            ...
            {
            case LUA_ERRRUN:
                sErrM = "Runtime error processing RGD \'%s\' : %s";
                break;
            case LUA_ERRMEM:
                sErrM = "Memory error processing RGD \'%s\' : %s";
                break;
            case LUA_ERRERR:
                sErrM = "Error error processing RGD \'%s\' : %s";
                break;
            default:
                sErrM = "Unknown error processing RGD \'%s\' : %s";
                break;
            }
            PAUSE_THROW(nullptr, __FILE__, __LINE__, sErrM, sFile, lua51_tostring(m_pL, -1));
            lua51_pop(m_pL, 1);
            UNPAUSE_THROW;
        }
    }
