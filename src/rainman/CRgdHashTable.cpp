...
static char *fgetline(FILE *f, unsigned int iInitSize = 32)
{
	...
	if (sBuffer == 0)
		throw new CRainmanException(0, __FILE__, __LINE__, "Failed to allocate %u", iInitSize);
...
void CRgdHashTable::ExtendWithDictionary(const char *sFile, bool bCustom)
{
	...
	if (fFile == 0)
		throw new CRainmanException(0, __FILE__, __LINE__, "Failed to open file \'%s\'", sFile);
	while (!feof(fFile))
	{
		char *sLine;
		try
