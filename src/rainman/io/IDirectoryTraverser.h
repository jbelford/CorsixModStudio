...
/*!
    ...
    \par Path names
    Firstly are entry points; these are "entry points" into the directory structure. For the file system, entry points
   are drive letters. For SGA archives they are TOC entries. <BR> A regex for file paths is [:entry
   point:](\[:directory:])*\[:file name:] <BR> A regex for directory paths is [:entry point:](\[:directory:])*\? <BR>
    Entry points, file names and directories can use any alphanumeric characters apart from '\0' and '\\', and any
   naming limitations imposed from the underlying system <BR>
*/
class RAINMAN_API IDirectoryTraverser
{
  public:
	...
	*/
	  ...
	  public:
		...
		*/
		...
		/*!
		    ...
		    \return Returns a valid pointer, although may return 0 for an empty sub-dir and will throw a
		   CRainmanException on error. Remember to delete this pointer when you are done using it.
		*/
		...
		/*!
		    ...
		    \return Returns a valid pointer, but will throw a CRainmanException on error. Remember to delete this
		   pointer when you are done using it.
		*/
	...
	/*!
	    \param[in] sPath The name of the directory to iterate. Read IDirectoryTraverser for information on directory
	   paths.
