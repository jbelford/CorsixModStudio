...
CRgmFile::CMaterial::CMaterial(CChunkyFile::CChunk *pChunk)
{
	...
	if (pChunk->GetVersion() != 1)
		throw new CRainmanException(0, __FILE__, __LINE__, "Material is version %lu, expected version 1",
		                            pChunk->GetVersion());
...
void CRgmFile::CMaterial::_ParseInfo(CChunkyFile::CChunk *pChunk)
{
	...
	if (pChunk->GetType() != CChunkyFile::CChunk::T_Data || strcmp(pChunk->GetName(), "INFO") != 0)
		throw new CRainmanException(__FILE__, __LINE__, "Not a material chunk");
	if (pChunk->GetVersion() != 1)
		throw new CRainmanException(0, __FILE__, __LINE__, "Material info is version %lu, expected version 1",
		                            pChunk->GetVersion());
...
CRgmFile::CMaterial::CVariable::CVariable(CChunkyFile::CChunk *pChunk)
{
	...
	if (pChunk->GetVersion() != 1)
		throw new CRainmanException(0, __FILE__, __LINE__, "Material variable info is version %lu, expected version 1",
		                            pChunk->GetVersion());
	...
	else
	{
		delete pData;
		_Free();
		throw new CRainmanException(0, __FILE__, __LINE__, "Data type %lu has not been seen before. CONTACT CORSIX",
		                            iDataType);
	}
