...
void CModuleFile::_DoLoadFolder(const char *sFullPath, bool bIsDefaultWrite, unsigned short iNum, const char *sTOC,
                                const char *sUiName, CALLBACK_ARG, bool *bIsWritable)
{
    if (bIsWritable != 0)
        *bIsWritable = false;
    // Get source name
    ...
    try
    {
        ...
        if (pDirItr)
        {
            ...
            if (bIsThisMod)
            {
                pSrc = m_pNewFileMap->RegisterSource(m_iFileMapModNumber, false, iNum, GetFileMapName(), sSlashChar,
                                                     m_pFSS, m_pFSS, m_pParentModule ? false : true, bIsDefaultWrite);
                if (bIsWritable != 0)
                    *bIsWritable = m_pParentModule ? false : true;
            }
            else
                pSrc = m_pNewFileMap->RegisterSource(15000, false, iNum, sActualModName, sSlashChar, m_pFSS, m_pFSS,
                                                     false, false);
...
void CModuleFile::_DoLoadArchive(const char *sFullPath, CSgaFile **ppSga, unsigned short iNum, const char *sUiName,
                                 CALLBACK_ARG)
{
    // Get source name
    const char *sSlashChar;
    ...
    try
    {
        inputStream = std::unique_ptr<IFileStore::IStream>(m_pFSS->VOpenStream(sFullPath));
    }
    catch (CRainmanException *pE)
    {
    ...
    if (m_eModuleType == MT_CompanyOfHeroes)
    {
        ...
        {
            if (m_saScenarioPackRootFolder &&
                strnicmp(m_saScenarioPackRootFolder, sFullPath, strlen(m_saScenarioPackRootFolder)) == 0)
            {
                bIsThisMod = false;
    ...
    try
    {
        ...
        pSga->Load(inputStream.get(), m_pFSS->VGetLastWriteTime(sFullPath));
        ...
        if (pDirItr)
        {
            void *pSrc;
            if (bIsThisMod)
                pSrc = m_pNewFileMap->RegisterSource(m_iFileMapModNumber, true, iNum, GetFileMapName(), sSlashChar,
                                                     pSga, pSga, false, false);
            else
                pSrc = m_pNewFileMap->RegisterSource(15000, true, iNum, sActualModName, sSlashChar, pSga, pSga, false,
                                                     false);
                                  ...
                                  unsigned long iReloadWhatEngines, CALLBACK_ARG)
{
    ...
    if (iReloadWhat & RR_DataFolders)
    {
        ...
        {
            for (std::vector<CCohDataSource *>::iterator itr = m_vDataSources.begin(); itr != m_vDataSources.end();
                 ++itr)
            {
                ...
                if (_IsToBeLoaded(*itr) && (**itr).m_sFolder && *(**itr).m_sFolder)
                {
                    ...
                    _DoLoadFolder(sFolderPath, bIsWrite, 15000 + (**itr).m_iNumber, (**itr).m_sToc, (**itr).m_sFolder,
                                  THE_CALLBACK, &(**itr).m_bCanWriteToFolder);
                    delete[] sFolderPath;
                }
            }
        }
    ...
    if (iReloadWhat & RR_DataArchives)
    {
        ...
        {
            for (std::vector<CCohDataSource *>::iterator itr2 = m_vDataSources.begin(); itr2 != m_vDataSources.end();
                 ++itr2)
            {
                (**itr2).m_bIsLoaded = _IsToBeLoaded(*itr2);
                if (_IsToBeLoaded(*itr2))
                {
                    for (std::vector<CArchiveHandler *>::iterator itr = (**itr2).m_vArchives.begin();
                         itr != (**itr2).m_vArchives.end(); ++itr)
                    {
                        ...
                        _DoLoadArchive(sArchivePath, &(**itr).m_pHandle,
                                       15000 + (500 * (**itr2).m_iNumber) + (**itr).m_iNumber, (**itr).m_sName,
                                       THE_CALLBACK);
                        delete[] sArchivePath;
                    }
                }
...
void CModuleFile::_DoLoadCohEngine(const char *sFolder, const char *sUiName, unsigned long iLoadWhat,
