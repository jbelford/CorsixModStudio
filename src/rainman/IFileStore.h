...
class RAINMAN_API IFileStore
{
  public:
	...
	*/
	class RAINMAN_API IOutputStream : public IStream
	{
	  ...
	  public:
		...
		/*!
		    Writes data to the current location of the stream; overwiriting existing data, or adding new data to the end
		   of the file.
		...
		*/
		...
		template <class T> inline void VWriteString(const T *sString, size_t (*pSizeFn)(const T *) = strlen)
		{
			if (sString != 0)
	...
	*/
	...
	/*!
	    \param[in] sIdentifier For most file stores this will be the file path (See IDirectoryTraverser for information
	   on file path naming)
	    \return Returns a valid stream or throws an exception. Cannot return zero. Remember to delete this pointer when
	...
	*/
	...
	/*!
	    ...
	    \param[in] sIdentifier For most file stores this will be the file path (See IDirectoryTraverser for information
	   on file path naming)
	    \param[in] bEraseIfPresent If set to true then when the stream is opened, if the file has any existing content
	   then that is destroyed
	    \return Returns a valid stream or throws a CRainmanException. Cannot return zero. Remember to delete this
