...
frmTabDialog::frmTabDialog(const wxString &sTitle)
    : wxDialog(wxTheApp->GetTopWindow(), -1, sTitle, wxPoint(0, 0),
               wxWindow::FromDIP(wxSize(620, 750), wxTheApp->GetTopWindow()),
               wxFRAME_FLOAT_ON_PARENT | wxCAPTION | wxCLOSE_BOX | wxRESIZE_BORDER | wxMAXIMIZE_BOX | wxSYSTEM_MENU)
{
    ...
    pTopSizer->Add(
        m_pTabs = new wxAuiNotebook(
            this, -1, wxPoint(0, 0), FromDIP(wxSize(620, 750)),
            (wxAUI_NB_DEFAULT_STYLE | wxAUI_NB_TAB_EXTERNAL_MOVE | wxAUI_NB_WINDOWLIST_BUTTON | wxNO_BORDER) &
                (~(wxAUI_NB_CLOSE_BUTTON | wxAUI_NB_CLOSE_ON_ACTIVE_TAB | wxAUI_NB_CLOSE_ON_ALL_TABS))),
        1, wxALL | wxEXPAND, 0);

    SetSizer(pTopSizer);
    pTopSizer->SetItemMinSize(m_pTabs, FromDIP(620), FromDIP(750));
...
wxAuiNotebook *frmTabDialog::GetTabs() { return m_pTabs; }
