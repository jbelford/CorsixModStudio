...
class CTextViewAction : public frmFiles::IHandler
{
  public:
	...
	virtual wxString VGetAction() const { return wxT("View as text file"); }

	virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
	{
		UNUSED(oFile);
		frmScarEditor *pForm;
		TheConstruct->GetTabs()->AddPage(pForm = new frmScarEditor(oParent, sFile, TheConstruct->GetTabs(), -1),
		                                 wxString().Append(OnlyFilename(sFile)), true);
				    ...
				    sFile.AfterLast('.').IsSameAs(wxT("rgt"), false))
				{
					...
					if (!bCountOnly)
					{
						...
						else
						{
							...
							IFileStore::IOutputStream *pOutStr =
							    TheConstruct->GetModule()->VOpenOutputStream(saFile, true);
...
class CRgtViewAction : public frmFiles::IHandler
{
  public:
	...
	virtual wxString VGetAction() const { return wxT("View RGT"); }
...
class CExtractFolderAction : public frmFiles::IHandler
{
  public:
	...
	virtual wxString VGetAction() const { return wxT("Extract all files in this folder"); }
...
class CRGDAction : public frmFiles::IHandler
{
  public:
	...
	virtual wxString VGetAction() const { return wxT("View as RGD file"); }
	...
	virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
	{
		...
		TheConstruct->GetTabs()->AddPage(
		    ...
		    wxString().Append(wxT("RGD")).Append(wxT(" [")).Append(OnlyFilename(sFile)).Append(wxT("]")), true);
...
{
  public:
	virtual wxString VGetExt() const { return wxT("mua"); }

	virtual wxString VGetAction() const { return wxT("View as LUA/MUA file"); }
	...
	virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
	{
		...
		if (pLua)
		{
			...
			TheConstruct->GetTabs()->AddPage(
			    pForm = new frmRGDEditor(oParent, sFile, TheConstruct->GetTabs(), -1),
			    wxString().Append(wxT("MUA")).Append(wxT(" [")).Append(OnlyFilename(sFile)).Append(wxT("]")), true);
			if (!pForm->FillFromMetaTable(pLua))
			{
				ErrorBox("Cannot load file");
				delete pLua;
...
class CLuaBurnAction : public frmFiles::IHandler
{
  public:
	...
	virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
	{
		UNUSED(oFile);
		long iRGDVersion = 1;
		if (TheConstruct->GetModule()->GetModuleType() != CModuleFile::MT_DawnOfWar)
			iRGDVersion = 3;
...
class CLuaBurnFolderIncReqAction : public frmFiles::IHandler
{
  ...
  public:
	CLuaBurnFolderIncReqAction() : m_pProgress(0)
	{
		m_iRGDVersion = 1;
		m_bCheckSkip = true;
		if (TheConstruct->GetModule()->GetModuleType() != CModuleFile::MT_DawnOfWar)
...
{
  public:
	virtual wxString VGetExt() const { return wxT("nil"); }
...
{
  public:
	virtual wxString VGetExt() const { return wxT("scar"); }

	virtual wxString VGetAction() const { return wxT("View as SCAR file"); }
	...
	virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
	{
		...
		TheConstruct->GetTabs()->AddPage(
		    pForm = new frmScarEditor(oParent, sFile, TheConstruct->GetTabs(), -1),
		    wxString().Append(wxT("SCAR")).Append(wxT(" [")).Append(OnlyFilename(sFile)).Append(wxT("]")), true);
