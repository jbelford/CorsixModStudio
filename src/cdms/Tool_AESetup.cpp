...
frmUCSToDAT::frmUCSToDAT()
    : wxDialog(wxTheApp->GetTopWindow(), -1, AppStr(aesetup_name), wxPoint(0, 0), wxDefaultSize,
               wxFRAME_FLOAT_ON_PARENT | wxFRAME_TOOL_WINDOW | wxCAPTION)
{
	...
	pTopSizer->Add(SBT(new wxStaticText(this, -1, AppStr(aesetup_outselect)), AppStr(aesetup_outselect_help)), 0,
	               wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL | wxFIXED_MINSIZE | wxALL, 3);
	pTopSizer->Add(SBT(m_pOutFile = new wxTextCtrl(this, IDC_FileOut, wxT("")), AppStr(aesetup_outselect_help)), 1,
	               wxALL | wxEXPAND, 3);
	pTopSizer->Add(
	    SBT(pBgTemp = new wxButton(this, IDC_BrowseOut, AppStr(aesetup_browse)), AppStr(aesetup_outselect_help)), 0,
	    wxALIGN_CENTER_VERTICAL | wxALIGN_LEFT | wxFIXED_MINSIZE | wxALL, 3);

	pTopSizer->Add(SBT(new wxStaticText(this, -1, AppStr(aesetup_range1)), AppStr(aesetup_range_help)), 0,
	               wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL | wxFIXED_MINSIZE | wxALL, 3);
	pTopSizer->Add(SBT(m_pRangeStart = new wxTextCtrl(this, IDC_FileOut, wxT("90000")), AppStr(aesetup_range_help)), 1,
	               wxALL | wxEXPAND, 3);
	m_pRangeStart->SetValidator(oValidator);
	pTopSizer->AddSpacer(0);

	pTopSizer->Add(SBT(new wxStaticText(this, -1, AppStr(aesetup_range2)), AppStr(aesetup_range_help)), 0,
	               wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL | wxFIXED_MINSIZE | wxALL, 3);
	pTopSizer->Add(SBT(m_pRangeEnd = new wxTextCtrl(this, IDC_FileOut, wxT("199999")), AppStr(aesetup_range_help)), 1,
	               wxALL | wxEXPAND, 3);
...
{
	UNUSED(event);
	m_pOutFile->SetValue(wxFileSelector(AppStr(aesetup_outselect), wxT(""), m_pOutFile->GetValue(), wxT("dat"),
	                                    AppStr(aesetup_filter), wxFD_SAVE | wxFD_OVERWRITE_PROMPT, TheConstruct));
}
...
void UCSToDATConvertor::setModule(const CModuleFile *pModule) { m_pModule = pModule; }
...
void UCSToDATConvertor::_endRange() { fprintf(m_fDAT, "rangeend\n"); }
