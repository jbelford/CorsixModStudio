...
//m_pLua2Object
	wxPGProperty* oName = m_pPropertyGrid->Append(new wxStringProperty(wxT("Name"), wxT("Name"), GetMetaNodeName(pNode)));
	m_pPropertyGrid->SetPropertyHelpString(oName, wxT("The name of this item"));
	if(m_pLua2Object) oName->Enable(false);
	...
	wxPGProperty* oVal;
	m_pPropertyGrid->SetPropertyHelpString(m_pPropertyGrid->Append(new wxEnumProperty(wxT("Data Type"), wxT("DataType"), asDataTypes, aiDataTypes, (int)pNode->VGetType())), wxT("What kind of value this is"));
	if(pTable)
	{
		m_pPropertyGrid->SetPropertyHelpString(oVal = m_pPropertyGrid->Append(GetMetaNodeEditor(pNode, wxT("Reference"), wxT("Value"))), GetMetaNodeHelp(pNode));
	...
	{
		m_pPropertyGrid->SetPropertyHelpString(oVal = m_pPropertyGrid->Append(GetMetaNodeEditor(pNode, wxT("Value"), wxT("Value"))), GetMetaNodeHelp(pNode));
	}
...
void frmRGDEditor::_DoValueChange(wxTreeItemId oTreeItem, wxPropertyGridEvent& event, wxPGProperty* oPropItem, bool bIsChild)
{ UNUSED(bIsChild);
