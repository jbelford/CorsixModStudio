...
//! Abstract interface for the DPS calculator's UI operations.
/*!
    Decouples CDpsCalculatorPresenter from the tool action so that the presenter
    can be tested with a mock view.
*/
class IDpsCalculatorView
{
  public:
    virtual ~IDpsCalculatorView() = default;
    ...
    //! Called when DPS calculation completed successfully.
    virtual void OnDpsComplete() = 0;
