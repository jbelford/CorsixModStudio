        ...
        for (size_t i = 0; i < iUCSCount; ++i)
        {
            try
            {
                ...
                if (TheConstruct->GetModuleService().GetModuleType() == CModuleFile::MT_CompanyOfHeroes)
                    ...
                    aList.Add(AsciiTowxString(pMod->GetFileMapName()) + wxT("\'s ") +
               ...
               wxFRAME_FLOAT_ON_PARENT | wxFRAME_TOOL_WINDOW | wxCAPTION)
{
    ...
    size_t iReqCount = TheConstruct->GetModuleService().GetRequiredCount();
    ...
    size_t iEngineCount = TheConstruct->GetModuleService().GetEngineCount();
...
bool frmUCSSelector::GotAnswer() { return m_bGotAnswer; }
...
void frmUCSSelector::OnNewClick(wxCommandEvent &event)
{
    UNUSED(event);
    wxString sVal;
    wxString sDefault = TheConstruct->GetModuleService().GetFileMapName();
    if (TheConstruct->GetModuleService().GetModuleType() == CModuleFile::MT_CompanyOfHeroes)
    {
        sDefault.Append(wxT("."));
        sDefault.Append(TheConstruct->GetModuleService().GetLocale());
    }
    sDefault.Append(wxT(".ucs"));
    sVal = wxGetTextFromUser(AppStr(ucsselect_newcaption), AppStr(ucsselect_new), sDefault, this, wxDefaultCoord,
                             wxDefaultCoord, false);
    if (!sVal.IsEmpty())
    {
        m_sAnswer = sVal;
        EndModal(wxID_NEW);
