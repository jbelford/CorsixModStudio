...
//! Abstract interface for the SGA creation dialog's UI operations.
/*!
    Decouples CSgaMakePresenter from frmSgaMake so that the presenter
    can be tested with a mock view.
*/
class ISgaMakeView
{
  public:
    virtual ~ISgaMakeView() = default;

    //! Show the loading/progress dialog with a message.
    ...
    //! Called when SGA creation completed successfully.
    virtual void OnSgaCreated() = 0;
