...
{
	std::unique_ptr<wxFileDialog> pFileDialog(new wxFileDialog(this, AppStr(mod_select_sga),
			wxT(""), wxT(""), AppStr(mod_sga_filter), wxFD_OPEN));
...
void ConstructFrame::DoLoadMod(wxString sPath, eLoadModGames eGame)
{
	...
	if(sPath == wxT(""))
	{
		...
		pFileDialog = new wxFileDialog(this, AppStr(mod_select_file),
			sDoWPath, wxT(""), AppStr(mod_file_filter), wxFD_OPEN);
		if(!pFileDialog)
		{
			return;
		}
