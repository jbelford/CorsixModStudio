...
bool frmRgdMacro::_callback_load(void* pTag, const char* sFile)
{
	...
	if(!pThis->m_bAllowLoad)
	{
		char* sFile2 = strdup(sFile);
		{
			...
			unsigned long iHashRoot = crc32_case_idt(0, (const unsigned char*)sFile2, std::min(pThis->m_iPathLen, (unsigned long)strlen(sFile2)));
...
{ UNUSED(event);
	if(m_bShowingOutput)
	{
		wxString sFilename = wxFileSelector(AppStr(rgdmacro_saveoutputmsg),AppStr(app_macrospath),wxT(""),wxT("txt"),AppStr(rgdmacro_saveoutputfilter),wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
	...
	{
		wxString sFilename = wxFileSelector(AppStr(rgdmacro_savemacromsg),AppStr(app_macrospath),wxT(""),wxT("lua"),AppStr(rgdmacro_loadmacrofilter),wxFD_SAVE|wxFD_OVERWRITE_PROMPT);
		if(!sFilename.IsEmpty())
		{
			wxFile oFile;
			oFile.Open(sFilename, wxFile::write);
...
{ UNUSED(event);
	wxString sFilename = wxFileSelector(AppStr(rgdmacro_loadmacromsg),AppStr(app_macrospath),wxT(""),wxT("lua"),AppStr(rgdmacro_loadmacrofilter),wxFD_OPEN|wxFD_FILE_MUST_EXIST);
	if(!sFilename.IsEmpty())
	{
		wxFile oFile;
