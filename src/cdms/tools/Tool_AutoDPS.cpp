...
[23:44] <Kresjah> GameData["special_attack_ext"]["special_attacks"]["special_attack_XX"]       everything after that is
whatever you'd have in any other weapons LUA [23:45] <Kresjah> Area affect, throw data, weapon damage (including ap,
min-max damage), etc. [23:45] <Kresjah> Extra special fields are: [23:45] <dreddnott> Tomb Spyder definitely has one for
close-combat, it murders people [23:45] <Kresjah>
GameData["special_attack_ext"]["special_attacks"]["special_attack_XX"]["chance"] = Chance in percentage if I'm not
mistaken [23:46] <Kresjah> GameData["special_attack_ext"]["special_attacks"]["special_attack_XX"]["duration"] = How long
in seconds the attack lasts (for the sake of syncing with anim and knowing when to go back to "normal behaviour")
...
[23:47] <Kresjah> The second one (random) telling how much longer (or even possibly earlier) it can start a special
attack to make timing seem more random [23:47] <Kresjah> Meaning it will launch one every 4-6 second [23:47] <Corsix> ok
...
search_again:
    ...
    for (unsigned long iTableChild = 0; iTableChild < iTableChildrenCount; ++iTableChild)
    {
        ...
        {
            if (strlen(pTableChild->VGetName()) == iNamePartLen &&
                strncmp(pTableChild->VGetName(), sNamePart, iNamePartLen) == 0)
            {
                goto got_child;
            }
        }
...
void AutoDPS_Internal::AutoDPS_FileForEach_Rgd(CRgdFile *pRgd, tAutoDPS_Package *pPackage, tAutoDPS_WeaponInfo *pWeapon)
{
    ...
    if (pWeapon->iDefaultAP > 100.0f)
        pWeapon->iDefaultAP = 100.0f;
    delete pNode;
    ...
    if (pWeapon->iReloadTime < 0.0001f)
        delete pWeapon;
    else
        pPackage->pWeapons.push_back(pWeapon);
...
void AutoDPS_Internal::AutoDPS_FileForEach(IDirectoryTraverser::IIterator *pFileItr, void *pTag)
{
    ...
    {
        std::unique_ptr<IFileStore::IStream> pIn(pFileItr->VOpenFile());
...
static size_t fwrites(FILE *f, const char *s) { return fwrite(s, 1, strlen(s), f); }
