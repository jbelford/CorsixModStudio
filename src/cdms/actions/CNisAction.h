...
class CNisAction : public frmFiles::IHandler
{
  public:
    virtual wxString VGetExt() const { return wxT("nis"); }

    virtual wxString VGetAction() const { return wxT("View as NIS file"); }
    ...
    virtual void VHandle(wxString sFile, wxTreeItemId &oParent, wxTreeItemId &oFile)
    {
        ...
        TheConstruct->GetTabs()->AddPage(
            ...
            wxString().Append(wxT("NIS")).Append(wxT(" [")).Append(OnlyFilename(sFile)).Append(wxT("]")), true);
