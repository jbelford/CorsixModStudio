...
void frmMassExtract::OnGoClick(wxCommandEvent &event)
{
	...
	if (TheConstruct->GetModule()->IsFauxModule())
	{
		...
		pSource = pModule->GetFileMap()->RegisterSource(0, false, 0, "", "", pModule->GetFileSystem(),
		                                                pModule->GetFileSystem(), true, true, true);

		const char *saPath = wxStringToAscii(sSaveDirectory);
		std::unique_ptr<IDirectoryTraverser::IIterator> pItr(pModule->GetFileSystem()->VIterate(saPath));
		delete[] saPath;
