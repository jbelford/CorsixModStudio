...
--- **Sounds will be taken from the data:sound/speech/nis/ folder.  If you wish to play a sound from another folder you must use an optional path (ie. 'speech/single_player/guardsman/morale')<BR><BR> If the speech file is not available, a placeholder time will be calculated for the message using the number of words and the message will be displayed for that length of time.<BR> Actors:<BR>     ACT.Gabriel, ACT.Isador, ACT.Matiel, ACT.Bale, ACT.Sindri, ACT.Orkamungus, ACT.Macha, ACT.Brom, ACT.Toth, ACT.ScoutMarine, ACT.TacticalMarine, ACT.SluggaBoy, ACT.Guardsmen, ACT.Ranger, ACT.Apothecary, ACT.Cultist, ACT.Nob, ACT.Grot, ACT.Corralis, ACT.Servitor, ACT.Warlock, ACT.Dark_Reaper_Exarch, ACT.EvilIsador<BR><BR> To play speech for a custom actor, pass in a table = { blueprint = "blueprint_name", icon = "data:icon_path.tga" } for the Actor.
---@param actor ActorTable
---@param locID integer
...
--- An example build table:<BR>	local build_table =<BR>	{<BR>		{"space_marine_squad_servitor", 1, 5},<BR>		{"space_marine_squad_scout", 2, 3}<BR>	}<BR> The first number in each entry is the starting value -- the lowest number gets built first<BR> The second number is the increment that happens each time the unit is built<BR> NOTE: Setting build demand table clobbers the class preferences
---@param cpu_player PlayerID
...
--- An example buildings table:<BR>	local buildings_table =<BR>	{<BR>		{"space_marine_vehicle_building", 2},<BR>		{"space_marine_listening_post", 1}<BR>	}<BR> The first argument is the building name, the second is the number of those buildings the AI can build
---@param cpu_player PlayerID
...
--- Use Prod_DoesListContainSquad etc to query the resulting production table.<BR><BR> Example:<BR>     local ProductionTable = { }<BR>     Prod_GetProductionList( "GRP_ANYENTITYGROUP", ProductionTable )<BR>     if(Prod_DoesListContainSquad("chaos_marine_squad", ProductionTable) then<BR>         -- There is a building in GRP_ANYENTITYGROUP that is currently producing a space marine Squad_CanSeeEntity<BR>     end
---@param egroupname string
---@param list table
...
--- Use Prod_AddSquadToList etc to add entries to the given production list.<BR><BR> Example:<BR>     local ProductionTable = { }<BR>     Prod_AddSquadToList( "chaos_squad_cultist", list )<BR>     Prod_AddSquadToList( "chaos_squad_cultist", list )<BR>     Prod_AddSquadToList( "chaos_squad_defiler", list )<BR>     if( Prod_IsProducing( "GRP_ANYENTITYGROUP", ProcutionTable, true ) then<BR>         -- Of all the buildings in GRP_ANYENTITYGROUP, they are producing 2 cultists and a defiler.<BR>     end
---@param egroupname string
---@param list table
---@param all boolean
---@return boolean
...
--- Example:<BR>     local bpTable = Util_MakeBlueprintTable( "chaos_marine_squad", "chaos_marine_squad", "chaos_squad_bloodthirster" )<BR>     --Check if SGroup contains ALL items from the list (two chaos marines and a bloodthirster)<BR>     local result = SGroup_ContainsBlueprints( "sg_MyGroup", bpTable, true )<BR><BR>     --Check if an SGroup contains ANY blueprints from the list     result = SGroup_ContainsBlueprints( "sg_MyGroup", bpTable, false )<BR><BR>     If the first parameter is a table, it will append entries to the existing table
---@param args Args
...
--- In most cases you will call this function from OnGameSetup and store the returned playerId in  a global variable.  The player index should be in the range [0 , (numPlayers-1)].<BR><BR> Valid player races are the shortnames of the race attrib files.<BR><BR> Example:<BR>	function OnGameSetup( )<BR>	    g_player1 = Setup_Player(0, "PlayerDisplayName", "ork_race", 1)<BR>     -- g_player2 and g_player3 will be on the same team (allies)<BR>     g_player2 = Setup_Player(1, "$20000", "space_marine_race", 2)<BR>     g_player3 = Setup_Player(2, "$20001", "eldar_race", 2)<BR>	end
...
--- Example obj_table:<BR>  obj_table = { title_id = 400009,	short_desc_id = 400010, help_tip_id = 400011 }
...
--- Must pass in a table of tga file references and localized string numbers.<BR>  Example:<BR>     local table1 = {"command_icons/stance_burn", "command_icons/stance_hold", "command_icons/stance_cease"}<BR>     "command_icons/stance_burn" = enabled icon, "command_icons/stance_hold" = disabled icon, "command_icons/stance_cease" = mouseover icon<BR>     local table2 = { "$830050", "$830051"}<BR>     "$830050" = title, "$830051" = description<BR><BR>     g_buttonID1 = Util_SGroupAbilityButton_Create(table1, table2, false)<BR>
---@param enabled boolean
---@return ButtonID
...
--- Must pass in a valid widget name and table of tga file references and localized string numbers.<BR>  Example:<BR>     local table1 = {"command_icons/stance_burn", "command_icons/stance_hold", "command_icons/stance_cease"}<BR>     local table2 = { "$830050", "$830051"}<BR><BR>     current acceptible widget names include : "btn_ScarUI"<BR><BR>     g_buttonID1 = Util_ScarButton_Create("btn_ScarUI", table1, table2, true, false)<BR>
...
--- The offset is a number from 0 to 7 and indicates a location around 'position' as follows:<BR>7   0   1<BR>6  pos  2<BR>5   4   3
---@param groupName string
---@param position Position
...
--- The the target SGroup can only have one squad in it. The offset is a number from 0 to 7 and indicates a location around the target sgroup (oriented based on that group's orientation) as follows:<BR>7   0   1<BR>6  pos  2<BR>5   4   3
---@param groupName string
...
--- Example:<BR>     Valid sound channels are either PC_Music or PC_Ambient<BR>     This utility function should be called once per channel<BR><BR>     local tracksTable = {"music_track1", "music_track2"}<BR>     Must use valid music or ambient track names from the \Warhammer\DataGeneric\W40k\Sound\Music or \Warhammer\DataGeneric\WXP\Sound\Music directory<BR><BR>     local silenceTable = {5, 20}<BR>     This example would set a random time of as little as 5 seconds but no more than 20 seconds between each track.<BR><BR>     The function call could look something like: <BR>     t_music = {"music_theme_chaos_01", "music_ork_theme", "music_coming_of_chaos" }<BR>     Playlist_Manager( PC_Music, t_music, true , {20, 40})<BR><BR>
...
--- Please note the only one level of sub objectives is supported (i.e.  an objective cannot be added as a sub objective of an existing sub objective)<BR><BR>  Example obj_table:<BR> obj_table = { title_id = 400009, short_desc_id = 400010, help_tip_id = 400011 }
---@param obj_table table
...
--- Example: Anim_PlaySyncAnim( 'sg_force_commander', 'sg_blood_thirster', 'SyncKill_7', 'SyncDeath_7'<BR>  Also, you will want to make sure the two squads are close to each other since the sync kill animation is going to 'pop' the units into place.
...
--- You can layer this on other Camera_ActivateEntityCam calls.<BR>  camera name - name of the camera in the model<BR> shake amount - amount of shake to allow (0-1.0)<BR> IMPORTANT: Blend time will have unpredictable results if blending to a moving entity or the camera is animated within the blend time.  Use with EXTREME caution!
...
--- blend time - time to blend to default camera<BR>
...
--- This function MUST be called through a CTRL object in NISlets if secs is greater then zero.<BR>
---@param egroupID EGroupID
---@param secs number
function Camera_FocusOnTargetEGroup(egroupID, secs) end

--- Focus the camera on a target position.
--- This function MUST be called through a CTRL object in NISlets if secs is greater then zero.<BR>  IMPORTANT: This function does not lock user input.  If this function is called through a CTRL Camera_SetInputEnabled(false) should almost certainly be called!
---@param targetpos Position
---@param secs number
...
--- Activate the specified camera within the entity.
--- You can layer this on other Camera_SGActivateEntityCam calls.<BR>  camera name - name of the camera in the model<BR> shake amount - amount of shake to allow (0-1.0)<BR> IMPORTANT: Blend time will have unpredictable results if blending to a moving entity or the camera is animated within the blend time.  Use with EXTREME caution!
...
--- epicentre - origin of camera shake.  as you move away from this origin, the shake is less intense.<BR>  timeSecs - duration of the shake<BR> intesity - intesity of the shake.<BR> radius - the extents of the shake from the epicentre<BR> falloff - the amount of shake that the camera loses as it gets further from the epicentre<BR><BR> NOTE: You should not call this function directly from your mission scripts.  You should use the functions in CameraShake.scar
---@param epicentre Position
...
--- Use this function to isssue an entity-based command to a squad group. (eg. order a squad to attack a building)<BR> See Command_Squad for a list of all the possible squadCommands
---@param player PlayerID
---@param sgroup SGroupID
---@param squadCommand integer
---@param target EGroupID
...
--- Use this function to isssue a squad-based command to a squad group. (eg. order a squad to attack another squad)<BR> See Command_Squad for a list of all the possible squadCommands
---@param player PlayerID
---@param sgroup SGroupID
---@param squadCommand integer
---@param target SGroupID
...
--- The preference works in an additive way. Thus, if the preferences are:<BR> Marker1:	10<BR> Marker2:	20<BR> Marker3:	5<BR> Then the probability that the next unit built gathers at marker3 is 5/(10+20+5) = ~14% chance<BR>
---@param player PlayerID
...
--- The building_name corresponds to the file name<BR> (e.g. "space_marine_vehicle_building" and not "machine_cult")
---@param player PlayerID
...
--- Components:<BR>	CT_Attacking<BR>	CT_Resourcing<BR>	CT_BuildUnits<BR>	CT_BuildBuildings<BR>	CT_BuildResearch<BR>	CT_BuildAddOns	CT_Tactics
---@param player PlayerID
---@param enable boolean
---@param ct integer
...
--- AD_Easy<BR> AD_Standard<BR> AD_Hard<BR> AD_Advanced<BR> AD_Insane<BR>
---@param player PlayerID
---@return integer
...
--- UnitClasses: <BR>     UC_LightInfantryLow <BR>     UC_LightInfantryMed <BR>     UC_LightInfantryHigh <BR>     UC_HeavyInfantryMed <BR>     UC_HeavyInfantryHigh <BR>     UC_VehicleLow <BR>     UC_VehicleMed <BR>     UC_VehicleHigh <BR>     UC_MonsterMed <BR>     UC_MonsterHigh <BR>     UC_Commander <BR><BR> The preference works in an additive way. Thus, if the preferences are:<BR> UC_LightInfantry:	10<BR> UC_HeavyInfantry:	20<BR> UC_Vehicle:			5<BR> Then the probability that a vehicle is built next is 5/(10+20+5) = ~14% chance<BR> NOTE: Setting class preferences clobbers the build demand table
---@param player PlayerID
...
--- There are currently 3 types of production items.  PQ_Squads, PQ_AddOns, and PQ_Research.  Any of these items can in a production queue.<BR><BR> The results of this function will vary depending on the production items type.  For PQ_Squads, the squad blueprint name will be returned, for PQ_AddOns the AddOn name will be returned, and for PQ_Research the research name will be returned.<BR><BR> Use Entity_GetProductionQueueItemType to get the type of a given production queue item if you are interested in such things.
---@param entity EntityID
---@param index integer
---@return string
...
--- This function will validate all blueprint names.<BR> For example, you can turn off pre-caching for g_Player1 (assuming this player is setup) by:<BR> g_Player1NoPrecacheTable =<BR> {<BR>     "chaos_greater_sacrificial_circle"<BR>     "chaos_thermo_plasma_generator"<BR>     "chaos_raptor"<BR>     "chaos_raptor_aspiring_champion"<BR>     "chaos_bloodthirster"<BR>     "chaos_rhino"<BR>     "chaos_obliterator"<BR>     "chaos_daemon_prince"<BR> }<BR><BR> Player_DoNotPrecache( g_Player1, g_Player1NoPrecacheTable )
---@param pPlayer PlayerID
...
--- Entity groups are used for buildings and objects such as rocks and trees.<BR> If you need to issue orders to a group vehicles or units you must use a SquadGroup.
---@param name string
---@return EGroupID
...
--- function Rule_Test( )<BR>     local DespawnEntity = function( egroupid, itemindex, entityID )<BR>         Entity_Despawn( entityID )<BR>     end<BR><BR>     EGroup_ForEach( EGroup_FromName("eg_PlayersHQ"), DespawnEntity )<BR> end<BR> Note: This function iterates over SPAWNED ENTITIES ONLY.
---@param egroup EGroupID
---@return boolean
...
--- This is used for checking if ALL or ANY items in a group match a given predicate. (eg. Are all items in a group choas marines)<BR> Note: This function iterates over SPAWNED ENTITIES ONLY.
---@param egroup EGroupID
---@param all boolean
---@return boolean
...
--- If 'group' does not contain an entity from 'groupToRemove', that entity will be skipped.<BR>  Example: Remove group2 (0, 10, 11, 22) from group1 (0, 10, 11, 12, 13, 14) --&gt; group1 would now be (12, 13, 14)
---@param group EGroupID
...
--- Check if a group contains ALL or ANY of the items in a blueprint table.
--- This function will validate all blueprint names.<BR> Set all to true to check for all blueprints in the blueprintTable or set to false to check for any.<BR> A blueprint table containing two marines and a bloodthirster looks like so:<BR> BlueprintTable = {<BR>     { name = "chaos_marine_squad", count = 2 }<BR>     { name = "chaos_squad_bloodthirster", count = 1 }<BR> }<BR><BR> You could also call Util_MakeBlueprintTable:<BR> BlueprintTable = Util_MakeBlueprintTable( "chaos_marine_squad", "chaos_marine_squad", "chaos_squad_bloodthirster")<BR><BR> This function will take care of setting up the table for you.
---@param sgroup SGroupID
---@param all boolean
---@return boolean
...
--- Squad groups are used for all units and vehicles.  You can issue orders such as move and attack to an entire squad group.  If you want a group for buildings or objects such as trees, use an EntityGroup instead.<BR>  Note that you are allowed to have unamed squadgroups or squadgroups with the samename.  Be careful not to create  duplicates of your squad group when you dont intend to.  To be safe, you can check if the SquadGroup you want to create  exists using SGroup_Exists( )
---@param name string
---@return SGroupID
...
--- Call a lua function for each item in a group. Function will recieve (groupid, itemindex, itemid) and should return true to break or false to continue.
--- function Rule_Test( )<BR>     local DespawnSquad = function( sgroupid, itemindex, squadID )<BR>         Squad_Despawn( squadID )<BR>     end<BR><BR>     SGroup_ForEach( SGroup_FromName("sg_Squads"), DespawnSquad )<BR> end<BR><BR> This functions iterates over spawned squads only.
---@param sgroup SGroupID
---@return boolean
function SGroup_ForEach(sgroup) end

--- Call a lua function for each item in a group. Function will recieve (groupid, itemindex, itemid) and should return a bool.
--- This is used for checking if ALL or ANY items in a group match a given predicate. (eg. Are all items in a group choas marines)<BR> This functions iterates over spawned squads only.
---@param sgroup SGroupID
---@param all boolean
---@return boolean
...
--- Race starting entities are given through the Forward Base mechanism<BR>  This function will throw an exception if called outside of a Meta Map game or an unknown race is specified.
---@return integer
function MetaMap_GetAttackingRaceStartingEntitiesCount() end

--- Returns the indexed starting entity for the attacking race.
--- Use the function MetaMap_GetRaceStartingEntitiesCount to determine the number of starting squads a race will be allocated.<BR>  This function will throw an exception if called outside of a Meta Map game, an unknown race is specified or an out of range index is given.
---@param index integer
---@return string
...
--- Use the function MetaMap_GetRaceStartingSquadsCount to determine the number of starting squads a race will be allocated.<BR>  This function will throw an exception if called outside of a Meta Map game, an unknown race is specified or an out of range index is given.
---@param raceName string
---@param index integer
---@return string
...
--- Race starting squads are given through several possible mechanisms, including the owned territories for   the race (and thus changes as a race gains or loses territories).<BR> This function will throw an exception if called outside of a Meta Map game or an unknown race is specified.
---@param raceName string
---@return integer
...
--- Opponent indices are 0 based.  The return value will be one of: AD_Easy, AD_Standard, AD_Hard, AD_Advanced or AD_Insane.<BR> This function will throw an exception if called outside of a Meta Map game.
---@param territoryIndex integer
...
--- applicationType : MAT_EntityType, MAT_Entity, MAT_Player, MAT_Squad, MAT_SquadType, or MAT_WeaponType.<BR> modtype : Name of modifier from the modifiers table in the attribute editor.<BR> usageType : MUT_Addition, MUT_Multiplication or MUT_Enable.<BR> exclusive : <BR> value : The value for the modifier.<BR> extname : The name of the entity/squad/weapon blueprint for when the applicationType is MAT_EntityType, MAT_SquadType or MAT_WeaponType.
...
--- In order to call this, you must have called CPath_Start() or one of the CPath_Cut functions<BR><BR>  This function can be called through a CTRL and waited on.
...
--- Only one camera path can be started at a time.  Once a camera path is started, it is ok to cut to a different path or to play sections from the same path.<BR><BR>  Once the frame specified by tokeyframe has been reached, the camera will pause and hold on that position until you cut to another section or it blends out using the blendout value.<BR><BR> The keyframe strings represent the names of the keyframes as specified in the ME.<BR><BR> This function can be called through a CTRL and waited on.
---@param cpath string
---@param fromkeyframe string
---@param tokeyframe string
...
--- The squad will move along the terrain regardless of the height of the path. The squad will also be removed from pathfinding while traversing the path.<BR><BR>  This function can be called through a CTRL object and waited on.
---@param sanimname string
---@param fromkeyframe string
---@param tokeyframe string
---@param sgroup SGroupID
...
--- This function works just like SPath_Start.  The speed override use this speed for all points. Set speed to zero to use default.<BR><BR>  Setting autoFinsih to true will automagically call SPath_Finish at the end of the path.
---@param sanimname string
---@param fromkeyframe string
---@param tokeyframe string
---@param sgroup SGroupID
...
--- RS_Locked - Research is restricted or not yet available.<BR> RS_Researching - Research is queued up in one of the players buildings.<BR> RS_Complete - Research has been granted or completed.<BR> RS_Available - Research is available to build in one or more of the players buildings
---@param player PlayerID
---@param researchName string
---@return integer
...
--- Possible resource types are RT_Requisition, RT_Power, RT_Pop.<BR>
---@param player PlayerID
---@param resourceType integer
---@param amt number
...
--- The leader of the squad will face the position and all other troops will use the same rotation as their leader.<BR><BR>  This function works on spawned squads only.
---@param squad SquadID
---@param pos Position
function Squad_FacePosition(squad, pos) end

--- Get 2 squads to face each other.
--- The leaders of the squad will face each other, and all troopers in the squad will use the same rotation as their leader.<BR><BR>  This function works on spawned squads only.
...
--- Returns a soundID you can use to stop the sound etc.<BR>  Example:<BR><BR>     local id = Sound_Play( "Speech/SpaceMarine/AddOnComplete" )<BR>     Sound_Stop( id )
---@param name string
---@return integer
...
--- Valid channel values: PC_Music or PC_Ambient <BR>  Valid tracks are the sound files from the Data:Sound/Music folder, without the file extension.
---@param channel SoundChannel
---@param track string
...
--- This will create a squad of size 'loadoutCount' and of unit type 'trooper_base' (from squad blueprint)<BR> Note: if loadoutCount is zero, the min loadout will be used.  loadoutCount will be clipped to the max loadout if it ever exceeds it.
---@param squadblueprint string
---@param player PlayerID
---@param pos Position
...
--- This function will do nothing if the squad is in SquadStateLoad (loading a transport)<BR><BR>  ebpname is the name of the building with the deepstrike_ext (ie space_marine_orbital_relay)<BR><BR> you should never assume that this function will deepstrike your units because there may not be enough room!
---@param sgroup SGroupID
---@param ebpname string
---@param pos Position
---@return boolean
...
--- Trooper index is in the range [0..n-1]<BR>  Returns CT_None if trooper does not have cover extension Returns one of the possible cover types: CT_None, CT_Light, CT_Heavy, CT_Negative, CT_Blocking, CT_Stealth
---@param squad SquadID
...
--- armourTypeName param must be a valid armour type (type_armour) from the Attribute Editor.<BR>
---@param squad SquadID
...
--- bindingname, bindingparams<BR> move_modal, not 0 - used<BR> attackmove_modal, 0 - not used<BR> attackground_modal, 0 - not used<BR> jump_modal, 0 - not used<BR> rampage_modal, 0 - not used<BR> unload_modal, 0 - not used<BR> attach_modal, 0 - not used<BR> player_resource, 0 - power resource, 1 - requistion resource, 2 - ork resource<BR> focus_on_next, 0 - next builder button, 1 - next military button, 2 - next research button<BR> dlg_objectives, 0 - not used<BR> build_structures_menu, 0 - not used<BR> command_infiltrate, 0 - not used<BR>
...
--- PQ_Research - itemname is the name of the research blueprint<BR> PQ_AddOn - itemname is the name of the addon blueprint<BR> PQ_Squad - itemname is the name of the squad blueprint<BR>
...
--- The event file must be prepended with "data:Art/Events" and the directory it's in (if not in the Events directory). The file extension can be omitted.<BR>  Example: World_FXEvent("data:Art/Events/Unit_Upgrade_Morale_FX/Reinforce_Trooper", World_Pos(0,0,0))
...
--- Example: <BR><BR>World_FXEvent("movedest" World_Pos(0,0,0))<BR>      Plays the MoveFX event at location(0,0,0)
---@param filename string
---@param pos Position
---@return integer
...
--- The event file must be prepended with "data:Art/Events" and the directory it's in (if not in the Events directory). The file extension can be omitted.<BR>  Example: World_FXEventSquad("my_group", "data:Art/Events/Unit_Upgrade_Morale_FX/Reinforce_Trooper")
---@param filename string
---@param sgroup SGroupID
...
--- The only time you should need this function is to start playing a NISLet.  If your NISLet is named NISLet01 you would call this function like so:<BR><BR>     Event_Start( NISLets.NISLet01, 0 )<BR><BR> Events are run until a call to Wait() is hit.  When the event reaches Wait() it will not continue it execution until all controllers it is waiting for are done.<BR> To add a controller to the block simply call the controller through the CTRL object.  For example, to call the Delay controller you should call CTRL.Delay(5). This will make the succesive call to Wait delay for 5 seconds.<BR><BR> Example: (NISLets are located in the .nis file for your mission)<BR> NISLets = {<BR>     MyNISLet = function ( )<BR><BR>         print("Yo")<BR>         CTRL.Event_Delay(3)<BR>         print("This will show up right after Yo, it will not wait for 3 seconds")<BR>         CTRL.Event_Delay(2)<BR>         Event_Wait() -- execution will now delay for 5 seconds (waiting for controllers)<BR><BR>         print("Yo, 5 seconds later")<BR><BR>     end<BR><BR> To trigger the above NISLet from a Rule or your mission script you would call Event_Start( NISLets.MyNISLet, 0 )<BR><BR> If you start an event that has higher priority then the current event playing, the current event will be cancelled.<BR><BR> If you start an event that has lower or equal priority then the current event playing, the event will be queued and it will play when the current event is complete.<BR><BR> Zero is the highest priority and Zero+N is lower priority.<BR><BR>
---@param f function
...
--- This init function will be called when scar is started up.<BR> **Important: Make sure you do not register two functions with the same name; the init function names should be unique.
---@param f function
function Scar_AddInit(f) end

--- Register a load function with the scar system.
--- This load function will be called when scar is started up while loading a savegame.<BR> **Important: Make sure you do not register two functions with the same name; the load function names should be unique.
